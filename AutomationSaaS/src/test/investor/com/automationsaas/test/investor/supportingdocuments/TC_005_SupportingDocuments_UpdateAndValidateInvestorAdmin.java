...
public class TC_005_SupportingDocuments_UpdateAndValidateInvestorAdmin extends Base {
	...
	public void validateInvestor() {
		...
		investorDetailsPage.clickSupportingDocument();
		investorDetailsPage.previewDocument();
		Generics.refreshPage(driver);
		investorDetailsPage.clickSupportingDocument();
		investorDetailsPage.downloadDocument();
		Generics.refreshPage(driver);
		investorDetailsPage.clickSupportingDocument();
		investorDetailsPage.uploadDocument(png_upload_file_path);
		Generics.refreshPage(driver);
		investorDetailsPage.clickSupportingDocument();
		investorDetailsPage.validateDocumentCount("Adhar Card", 4);
		Generics.refreshPage(driver);
		investorDetailsPage.clickSupportingDocument();
		investorDetailsPage.deleteDocument();
		Generics.refreshPage(driver);
		investorDetailsPage.clickSupportingDocument();
		investorDetailsPage.validateDocumentCount("Adhar Card", 3);
		...
		supportingDocuments_DocumentName.add("JPGFile.jpg, TextFile.txt");
		...
		for (int i = 0; i < supportingDocuments_SerialNumber.size(); i++) {
			investorDetailsPage.validateSupportingDocuments(i, supportingDocuments_SerialNumber.get(i),
					supportingDocuments_DocumentCategory.get(i), supportingDocuments_DocumentName.get(i),
					supportingDocuments_VisibleToInvestor.get(i));
		}
	}
}