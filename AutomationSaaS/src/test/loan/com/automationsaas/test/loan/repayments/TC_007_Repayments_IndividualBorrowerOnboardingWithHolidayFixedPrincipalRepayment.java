...
public class TC_007_Repayments_IndividualBorrowerOnboardingWithHolidayFixedPrincipalRepayment extends Base {
	...
	public void validateIndividualBorrowerOnboarding() {
		...
		ReadWriteExcel.putData(excel_path, borrower_sheet_name, 1, 4, individualEmailId);
		...
		applyForLoanPage.selectLoanProduct("Car Loan");
		...
		applyForLoanPage.selectLoanRepaymentType("Holiday fixed Principal");
		try {
			Thread.sleep(2000);
		} catch (InterruptedException e) {
			e.printStackTrace();
		...
		ReadWriteExcel.putData(excel_path, borrower_sheet_name, 2, 4, borrowerId);
		...
		ReadWriteExcel.putData(excel_path, borrower_sheet_name, 3, 4, loanId);
