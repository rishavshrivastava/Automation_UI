...
public class TC_004_Repayments_IndividualBorrowerOnboardingWithFlatRateRepayment extends Base {
	...
	public void validateIndividualBorrowerOnboarding() {
		...
		ReadWriteExcel.putData(excel_path, borrower_sheet_name, 1, 3, individualEmailId);
		...
		applyForLoanPage.selectLoanProduct("Education");
		...
		applyForLoanPage.selectLoanRepaymentType("Flat Rate");
		try {
			Thread.sleep(2000);
		} catch (InterruptedException e) {
			e.printStackTrace();
		...
		ReadWriteExcel.putData(excel_path, borrower_sheet_name, 2, 3, borrowerId);
		...
		ReadWriteExcel.putData(excel_path, borrower_sheet_name, 3, 3, loanId);
