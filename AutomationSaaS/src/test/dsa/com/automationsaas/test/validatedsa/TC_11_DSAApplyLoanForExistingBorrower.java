...
public class TC_11_DSAApplyLoanForExistingBorrower extends Base {

	@Test(retryAnalyzer = RetryAnalyzer.class)
	public void validateApplyLoanForExistingBorrower() {
		...
		searchBorrowerDetails.add(emailId);

		for (int i = 0; i < searchBorrowerDetails.size(); i++) {
			existingBorrowerPage.validateSearchedBorrower(i, searchBorrowerDetails.get(i));
		}
		existingBorrowerPage.clickOnApplyForLoanLink();
		...
		detailsPage.enterWorkExperience("5");
		detailsPage.selectEmploymentCategory("Permanent");
		detailsPage.clearGrossIncome();
		...
		referenceDetailsPage.enterLastName("Yaragall");
		referenceDetailsPage.enterEmailAddress("chandrasekar.y@perfios.com");
		referenceDetailsPage.enterMobileNumber("8000000002");
		referenceDetailsPage.enterRelationshipStatus("Single");
		...
		String activeLoanID = dashboardPage.verifyAndCopyActiveLoanID(borrowerId);
		System.out.println("Loan ID created : " + activeLoanID);
		ReadWriteExcel.putData(excel_path, dsa_sheet_name, 9, 1, activeLoanID);
